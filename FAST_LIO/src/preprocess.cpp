...
#include "nano_flann/nanoflann_pcl.hpp"
...
Eigen::Vector3f Preprocess::computeNormal(const std::vector<Eigen::Vector3f> &neighbors)
{
    //  计算质心  计算协方差
      ...
      for (size_t i = r.begin(); i != r.end(); ++i)
      {
        ...
        };
        {
          ...
          if (nano_kdtree.radiusSearch(pt, radius, pointIdxRSearch, pointRSquaredDistance) > 5)
          {
            ...
            PointType added_pt;
            added_pt.normal_x = normal.x();
            added_pt.normal_y = normal.y();
            added_pt.normal_z = normal.z();
            added_pt.x = pl_orig.points[i].x;
            added_pt.y = pl_orig.points[i].y;
            added_pt.z = pl_orig.points[i].z;
            added_pt.intensity = pl_orig.points[i].intensity;
