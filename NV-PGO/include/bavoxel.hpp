...
public:
  ...
  void push_voxel(const vector<PointCluster> *vec_orig, const PointCluster *fix, Eigen::Vector3d &center,
                  Eigen::Vector3d &direct, Eigen::Vector3d &evalue, pcl::PointCloud<PointType>::Ptr plptr = nullptr)
  {
    int process_size = 0;
    for (int i = 0; i < win_size; i++)
      if ((*vec_orig)[i].N != 0)
...
public:
  ...
  int push_state; //  是否进行优化
  int layer;
  vector<P3V> vec_orig, vec_tran; //  存储local和global下的点坐标
  ...
  bool judge_eigen(int win_count)
  {
    ...
    vector<PointCluster> covMats(8); //  拆分
    for (int i = 0; i < win_count; i++)
    {
  ...
  {
    P3V &pvec_orig = vec_orig[ci];
    P3V &pvec_tran = vec_tran[ci];
    ...
    for (uint j = 0; j < a_size; j++)
    {
      int xyz[3] = {0, 0, 0};
      for (uint k = 0; k < 3; k++)
        if (pvec_tran[j].point[k] > voxel_center[k])
          xyz[k] = 1;
      int leafnum = 4 * xyz[0] + 2 * xyz[1] + xyz[2];
  ...
  void cut_func_normal(int win_count)
  {
    int point_size = 0;
    for (int i = 0; i < win_count; i++)
    ...
    int num = 0;
    //   1. normal matrix
    Eigen::MatrixXf normal_matrix(point_size, 3);
    for (int ci = 0; ci < win_count; ci++)
    {
      ...
      {
        normal_matrix(num, 0) = pvec_tran[i].normal[0];
        normal_matrix(num, 1) = pvec_tran[i].normal[1];
        normal_matrix(num, 2) = pvec_tran[i].normal[2];
        num++;
      }
    }
    // std::cout << __LINE__ << std::endl;
    //  2.  PCA
    Eigen::Vector3f mean = normal_matrix.colwise().mean(); //  mean
    Eigen::MatrixXf centered = normal_matrix.rowwise() - mean.transpose();
    Eigen::MatrixXf cov = (centered.transpose() * centered) / float(normal_matrix.rows() - 1);

    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXf> eigen_solver(cov);
    Eigen::Vector3f principal_component = eigen_solver.eigenvectors().col(2); //    最大特征值对应的特征向量，作为主方向（X轴）
    // std::cout << __LINE__ << std::endl;
    //  3.  Y/Z轴
    Eigen::Vector3f arbitrary_vector(1.0, 0.0, 0.0);           // 任意不平行x轴的向量
    if (fabs(principal_component.dot(arbitrary_vector)) > 0.9) //  若和主方向接近，换一个
    {
      arbitrary_vector = Eigen::Vector3f(0.0, 1.0, 0.0);
    }
    Eigen::Vector3f y_axis = principal_component.cross(arbitrary_vector).normalized(); //   Y轴为任意向量和X轴叉积
    Eigen::Vector3f z_axis = principal_component.cross(y_axis).normalized();           //   Z轴为X轴和Y轴的叉积

    //    4.  获取到新坐标系下的变换矩阵
    Eigen::Matrix3f rotation_matrix;
    rotation_matrix.col(0) = principal_component;
    rotation_matrix.col(1) = y_axis;
    rotation_matrix.col(2) = z_axis;
    Eigen::Matrix3d rotation_matrix_d = rotation_matrix.cast<double>();
    // std::cout << __LINE__ << std::endl;

    //    5.  转换到新坐标系下，并在新坐标系下进行分类
    for (int ci = 0; ci < win_count; ci++)
    {
      P3V &pvec_orig = vec_orig[ci];
      P3V &pvec_tran = vec_tran[ci];
      ...
      {
        point3D pt = pvec_tran[j];
        point3D pt_aft = pt;
        pt_aft.point = rotation_matrix_d.transpose() * pt.point;
        pt_aft.normal = rotation_matrix_d.transpose() * pt.normal;
        Eigen::Vector3d normal = rotation_matrix_d.transpose() * pt.normal;
        normal.normalize();
        // 计算法向量在X、Y、Z轴上的投影绝对值
        float x_proj = fabs(normal(0));
        float y_proj = fabs(normal(1));
        float z_proj = fabs(normal(2));
        // 比较三个轴上的投影大小，将法向量分入投影最大的轴方向
        int leafnum = 0;
        if (x_proj >= y_proj && x_proj >= z_proj)
        {
          if (normal(0) > 0) // X轴正方向
          {
            leafnum = 0;
            ...
            {
              leaves[leafnum] = new OCTO_TREE_NODE();
              leaves[leafnum]->voxel_center[0] = voxel_center[0];
              leaves[leafnum]->voxel_center[1] = voxel_center[1];
              leaves[leafnum]->voxel_center[2] = voxel_center[2];
              leaves[leafnum]->quater_length = quater_length;
              leaves[leafnum]->layer = layer + 1;
            }
        ...
        else if (y_proj >= x_proj && y_proj >= z_proj)
        {
          if (normal(1) > 0) // Y轴正方向
          {
            leafnum = 2;
    ...
    {
      if (leaves[tt] == nullptr)
        continue;
      int point_size = 0;
      for (int i = 0; i < win_count; i++)
        point_size += leaves[tt]->sig_orig[i].N;
      //  judge plane

      PointCluster covMat = leaves[tt]->fix_point;
      for (int i = 0; i < win_count; i++)
        covMat += leaves[tt]->sig_tran[i];

      Eigen::SelfAdjointEigenSolver<Eigen::Matrix3d> saes(covMat.cov());
      double plane_ratio = saes.eigenvalues()[0] / saes.eigenvalues()[1]; //  特征值从小到大排序

      if (point_size > min_ps && plane_ratio < 0.1)
      {
        leaves[tt]->push_state = 1;
        leaves[tt]->octo_state = 2;
      }
    }
    // std::cout << __LINE__ << std::endl;
  }

  void save_voxel(int win_count) //  存储第一层每个voxel的数据
  {
    std::string prename = "/home/myx/Fighting/Indoor-Multi-Session/src/Indoor_Multi_session/PCD/first_layer/";
    static int vox_index = 0;
    pcl::PointCloud<PointType>::Ptr plptr(new pcl::PointCloud<PointType>());
    for (int i = 0; i < win_count; i++)
    {
      PointType ap;
      for (auto &pvec : vec_tran[i])
      {
        ...
        ap.y = pvec.point[1];
        ap.z = pvec.point[2];
        ap.intensity = pvec.intensity;
        ap.normal_x = pvec.normal[0];
        ap.normal_y = pvec.normal[1];
        ap.normal_z = pvec.normal[2];
        plptr->push_back(ap);
      }
    }
    std::string filename = prename + std::to_string(vox_index++) + ".pcd";
    pcl::io::savePCDFileBinaryCompressed(filename, *plptr);
  }

  void recut_normal(int win_count)
  {
    if (octo_state == 0)
    {
      int point_size = fix_point.N;
      for (int i = 0; i < win_count; i++) //  计算pointcluster点的总个数
        point_size += sig_orig[i].N;
      if (point_size <= min_ps)
        return;
      //  TODO: 按6个面划分

      cut_func_normal(win_count);
      octo_state = 1; //  会遍历其分支叶子节点
      recut(win_count); 
    }
  }
  ...
  void recut(int win_count)
  {
    if (octo_state != 1)
    {
      ...
      {
        if (octo_state == 0 && point_size > layer_size[layer]) //  满足平面，设置为叶子节点，不在继续分了
          octo_state = 2;
      ...
      else if (layer == layer_limit) //  到达最大深度，也不再继续分了
      {
        octo_state = 2;
        return;
      }
      ...
      cut_func(win_count - 1); //  不执行

    for (int i = 0; i < 8; i++)
      if (leaves[i] != nullptr)
        leaves[i]->recut(win_count); //  递归，继续分
  }
  ...
  void tras_opt(VOX_HESS &vox_opt, int win_count)
  {
    if (octo_state != 1)
    {
      ...
      std::string prename = "/home/myx/Fighting/Indoor-Multi-Session/src/Indoor_Multi_session/PCD/voxel/";
      static int file_index = 0;
      if (push_state == 1)
      {
        vox_opt.push_voxel(&sig_orig, &fix_point, decision, layer);
        if (0) //  存储当前voxel数据，查看数据分布
        {
          pcl::PointCloud<PointType>::Ptr plptr(new pcl::PointCloud<PointType>());
          for (int i = 0; i < win_count; i++)
          {
          ...
          std::string filename = prename + std::to_string(file_index++) + ".pcd";
          pcl::io::savePCDFileBinaryCompressed(filename, *plptr);
        }
      }
    }
...
public:
  bool is2opt;
  int life;
  vector<int> each_num; //  存储每个pose的lidar数据在对应voxel中有多少个点
...
void cut_voxel(unordered_map<VOXEL_LOC, OCTO_TREE_ROOT *> &feat_map, pcl::PointCloud<PointType> &pl_feat, const IMUST &x_key, int fnum)
{
  ...
  {
    point3D pvec_orig(p_c);
    point3D pvec_tran = transPoint3D(pvec_orig, x_key);

    for (int j = 0; j < 3; j++)
    {
      loc_xyz[j] = pvec_tran.point[j] / voxel_size;
      if (loc_xyz[j] < 0)
        loc_xyz[j] -= 1.0;
    }
