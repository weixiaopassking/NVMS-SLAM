...
using PonintMapType = std::vector<std::vector<point3D>>;
using keyframePosesType = std::vector<Pose6D>;

struct MapmanagerType
{
    PonintMapType keyframePointClouds;
    ...
    MapmanagerType(const MapmanagerType &other)
    {
        ...
        keyframePointClouds = other.keyframePointClouds;
    ...
    MapmanagerType(MapmanagerType &&other) noexcept
    {
        ...
        keyframePointClouds = std::move(other.keyframePointClouds);
...
public:
    ...
    pcl::PointCloud<pcl::PointXYZI> addkeyframewithScancontext_density(KeyframepointType Keyframe, std::vector<point3D> key_points,Pose6D Pose_curr, double KeyframeTimes)
    {
        pcl::PointCloud<pcl::PointXYZI> vis_points;
        ActiveMap->keyframeClouds.push_back(Keyframe);
        ActiveMap->keyframePointClouds.push_back(key_points);
        ActiveMap->keyframeposes.push_back(Pose_curr);
        vis_points = ActiveMap->scmanager.makeAndSaveScancontextAndKeys_density(*Keyframe);
    ...
    pcl::PointCloud<pcl::PointXYZI> addkeyframewithScancontext(KeyframepointType Keyframe, std::vector<point3D> key_points,Pose6D Pose_curr, double KeyframeTimes)
    {
        pcl::PointCloud<pcl::PointXYZI> vis_points;
        ActiveMap->keyframeClouds.push_back(Keyframe);
        ActiveMap->keyframePointClouds.push_back(key_points);
        ActiveMap->keyframeposes.push_back(Pose_curr);
        vis_points = ActiveMap->scmanager.makeAndSaveScancontextAndKeys(*Keyframe);
        ActiveMap->keyframeTimes.push_back(KeyframeTimes);
        ActiveMap->keyframeposesUpdated.push_back(Pose_curr);
    ...
    void clear()
    {
        ...
        ActiveMap->keyframePointClouds.clear();
...
#pragma omp parallel for num_threads(8)
        for (size_t i = 0; i < ActiveMap->keyframeposes.size(); ++i)
        {
    ...
    void mergemap()
    {
        ...
        ActiveMap->keyframePointClouds.insert(ActiveMap->keyframePointClouds.begin(),
                                        SleepMapList[Wakeup_Mapindex]->keyframePointClouds.begin(),
                                        SleepMapList[Wakeup_Mapindex]->keyframePointClouds.end());
