...
struct point3D
{
  ...
  point3D(PointType &p)
  {
    this->point = Eigen::Vector3d(p.x, p.y, p.z);
    this->normal = Eigen::Vector3d(p.normal_x, p.normal_y, p.normal_z);
...
point3D transPoint3D(point3D &pt, const IMUST &x_key)
{
  point3D res(pt);
  res.point = x_key.R * pt.point + x_key.p;
  res.normal = x_key.R * pt.normal;
  return res;
}

void down_sampling_voxel(pcl::PointCloud<PointType> &pl_feat, double voxel_size) //  均值采样
{
  if (voxel_size < 0.001)
    return;
  ...
  for (PointType &p_c : pl_feat.points)
  {
    ...
    else
    {
      ...
      pp.x = (pp.x * pp.curvature + p_c.x) / (pp.curvature + 1); //  求平均了
...
void down_sampling_voxel0(pcl::PointCloud<PointType> &pl_feat, double voxel_size) //  取第一个点
{
  if (voxel_size < 0.001)
    return;
...
void plvec_trans(P3V &porig, P3V &ptran, IMUST &stat)
{
  uint asize = porig.size();
  ptran.resize(asize);
  for (uint i = 0; i < asize; i++)
  {
    ptran[i].point = stat.R * porig[i].point + stat.p;
    ptran[i].normal = stat.R * porig[i].normal;
  }
}
...
public:
  ...
  void push(const point3D &vec)
  {
    N++;
    P += vec.point * vec.point.transpose();
    v += vec.point;
  }
