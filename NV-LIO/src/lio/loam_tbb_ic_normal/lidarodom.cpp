...
namespace zjloc::loam_tbbic_normal
{
     ...
     lidarodom::lidarodom(/* args */)
     {
          ...
          index_frame = 1;
          points_world.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
          points_world_map.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
          points_lidar.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
     }
     ...
     void lidarodom::re_slam(void){
          ...
          points_world_map.reset(new pcl::PointCloud<pcl::PointXYZINormal>());
          current_state = new loam::state(Eigen::Quaterniond::Identity(), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero());
          imu_need_init_ = true;   // 是否需要估计IMU初始零偏
          flg_reset = true;
          index_frame = 1;
          voxel_map.clear();
          grids_cache_.clear();
     ...
     void lidarodom::run()
     {
          while (true)
          {
               ...
               for (auto &m : measurements)
               {
                    ...
                    {
                         flg_reset = false;
                         last_time = m.lidar_begin_time_;
                         continue;
                    }
                    current_time = m.lidar_begin_time_;
                    if (fabs(current_time - last_time) > 3.0)
                    {
                         ...
                         if (!hasPubsleep)
                         {
                              ...
                              std::string laser_topic = "Issleeping";
                              pub_bool_to_ros(laser_topic, issleeping, current_time);
                              cout << "Time changes too long, publish hibernating map operation!!!!!!!!!!!!!" << fabs(current_time - last_time) << endl;
                              hasPubsleep = true;
                         }
                         re_slam();
                         continue;
                    ...
                    else
                    {
                         ...
                         if (hasPubsleep)
                         {
                              if (!hasPubbuild)
                              {
                                   cout << "New session has happened, construct a new map....................." << endl;
                                   isPublish = true;
                                   std_msgs::Bool isbuilding;
                                   isbuilding.data = true;
                                   std::string laser_topic = "Isbuilding";
                                   pub_bool_to_ros(laser_topic, isbuilding, current_time);
                                   hasPubbuild = true;
                              }
                              hasPubsleep = false;
                         }
                                   ...
                                   int min_num_points)
     {
          ...
          addPointToPcl(points_lidar, points_world, pt, pt.intensity);
     ...
     void lidarodom::addPointToPcl(pcl::PointCloud<pcl::PointXYZINormal>::Ptr pcl_points_lidar, pcl::PointCloud<pcl::PointXYZINormal>::Ptr pcl_points_world, const point3D &pt, const double &intensity)
     {
          pcl::PointXYZINormal cloudTemp;
          cloudTemp.x = pt.point.x();
          cloudTemp.y = pt.point.y();
          cloudTemp.z = pt.point.z();
          cloudTemp.intensity = intensity;
          cloudTemp.normal_x = pt.normal.x(); 
          cloudTemp.normal_y = pt.normal.y(); 
          cloudTemp.normal_z = pt.normal.z();
          // cloudTemp.intensity = 50 * (point.z() - p_frame->p_state->translation.z());
          pcl_points_world->points.push_back(cloudTemp);

          pcl::PointXYZINormal cloudTemp_lidar;
          cloudTemp_lidar.x = pt.raw_point.x();
          cloudTemp_lidar.y = pt.raw_point.y();
          cloudTemp_lidar.z = pt.raw_point.z();
          cloudTemp_lidar.intensity = intensity;
          cloudTemp_lidar.normal_x = pt.raw_normal.x(); 
          cloudTemp_lidar.normal_y = pt.raw_normal.y(); 
          cloudTemp_lidar.normal_z = pt.raw_normal.z();
          pcl_points_lidar->points.push_back(cloudTemp_lidar);
     }
     ...
     void lidarodom::map_incremental(loam::cloudFrame *p_frame, int min_num_points)
     {
          ...
          {
               std::string laser_topic = "laser";
               pub_normalcloud_to_ros(laser_topic, points_world, p_frame->time_frame_end);
          }
          {
               std::string laser_topic = "lidar";
               pub_normalcloud_to_ros(laser_topic, points_lidar, p_frame->time_frame_end);
          }
          points_world->clear();
          points_lidar->clear();
     ...
     void lidarodom::setFunc(std::function<bool(std::string &topic_name,
                                                pcl::PointCloud<pcl::PointXYZINormal>::Ptr &cloud, double time)> &fun)
     {
          pub_normalcloud_to_ros = fun;
     }
     void lidarodom::setFunc(std::function<bool(std::string &topic_name,
     ...
     void lidarodom::setFunc(std::function<bool(std::string &topic_name,
                                                std_msgs::Bool isbegin_seeeion, double time)> &fun)
     {
          pub_bool_to_ros = fun;
     }
}
